...
我们证实，DiskANN所构建的SSD基索引可同时满足大规模ANNS的三大核心需求：**高召回率、低查询延迟、高密度（每个节点可索引的数据量）** 。在十亿级SIFT1B数据集上，DiskANN在一台16核机器上的**单秒查询处理量超5000次**，均值延迟不足3毫秒，且**1-recall@1高达95%以上**。而当前类似内存占用下的十亿级ANNS算法（如FAISS与IVFOADC+P ），其1-recall@1效率仅止步于50%左右。此外，与HNSW、NSG等当前主流的图基方法相比，DiskANN在高召回率场景下，每个节点可索引与检索的数据量是它们的5-10倍。最终，作为DiskANN完整架构的核心组件，我们提出了一款名为**Vamana**的全新图基ANNS索引，它的灵活性远超现有主流图索引算法——即使与全内存基的图索引相比亦是如此。
# 介绍
...
## 相关邻域图与贪婪搜索算法
大多数基于图的ANNS算法按以下方式工作：
在索引构建时期，它们根据数据集P的几何特性构建一个图G=(P,E)。
在搜索时期，针对查询向量xq​，算法在图G上采用自然的**贪婪搜索**或**最佳优先遍历（best-first traversal）** 策略（如算法 1 所示）。从某个指定的起始点s∈P出发，通过遍历图结构逐渐逼近xq​。
...
如下图所示，这个过程中实际上是把`pp*`的垂直平分线（图中虚线）的右半部分其它点如`p'`从p的领域中移除，也就是下面的剪枝算法algorithm2
[![Uploading file...k6dlz](https://raw.githubusercontent.com/waibibab-cs/blog_img/main/cdnimg/20260426155233.png)
...
如下图所示，这个过程相较于algorithm2的区别是减少了要剪枝的边的数量（导致图的平均度数增加），这本质上是空间与时间的权衡：保留长边意味着搜索时延降低，但图存储开销增大。下面第一个虚线是垂直平分线，第二个代表意味着α>1时，多了一些可以保留的长程边
![image.png](https://raw.githubusercontent.com/waibibab-cs/blog_img/main/cdnimg/20260426155615.png)
> 解释2：为什么用这种方式建立出边集合，在搜索路径上的每个节点处，与查询点之间的距离都能以α>1的倍数因子递减？
> 在上述“解释1”的基础上进行回答：仅考虑情况2，此时存在p1使得d(s,p) >=αd(p1,p)，其中α>1（当α=1时实际上就是SNG的构图策略），于是当s往p1遍历时，到p的距离会缩小至原来的1/β，(β>=α>1)，同理，再往下一个点遍历的时候，到p的距离会缩小至最开始的$1/{β^2}$ ，因此将从SNG的不确定的收敛速度提升至对数级收敛速度。下图展示了这个原理：
> ![image.png](https://raw.githubusercontent.com/waibibab-cs/blog_img/main/cdnimg/20260426155721.png)
...
我们的想法很简单：与其在查询阶段将请求路由到多个分片，不如**将每个基础点分配给多个附近的中心，以获得重叠的簇** ？事实上，我们首先使用k-means 聚类将十亿点级别的数据集划分为k个簇（假设k=40），然后将每个基础点分配给与其最近的ℓ个中心（通常ℓ=2就足够了）。接着，我们为分配给每个簇的点构建 Vamana 索引（每个簇的点数现在仅约为Nℓ/k，因此可以在内存中构建索引），最后通过简单地将所有边取并集（如下图所示，对于已经算好的两个聚类，在第一个聚类中A有两个边分别连接B1B2，第二个聚类中A分别连接C1C2，只需要求每个顶点在不同聚类中的并集就可以将不同的聚类连通起来），将这些不同的图合并为一个单一的图。经验表明，不同簇的这种重叠特性为GreedySearch算法提供了足够的连通性，使其能够成功找到目标，即使查询点的最近邻实际上分散在多个分片中也是如此。我们想指出的是，早期已有研究曾通过合并多个较小的、重叠的索引来为大型数据集构建索引。然而，他们构建重叠簇的方法与我们不同，对这些不同技术进行更详细的对比研究还有待完成。
![image.png](https://raw.githubusercontent.com/waibibab-cs/blog_img/main/cdnimg/20260426160104.png)
...
>对整个搜索过程的全面理解（大致上遵循algorithn1）：
>第一阶段：初始化
>1. 当一个新的查询向量 $x_q$ 输入系统时，搜索会从整个数据集的几何中心点（medoid）$s$ 开始 。
>2. 为了极大地减少初始阶段的磁盘访问，DiskANN 会在内存中缓存一部分频繁访问的节点 。具体来说，系统通常会把距离起点 $s$ 在 3 到 4 跳（hops）以内的所有节点的数据（包括全精度向量和邻居列表）直接缓存在内存中 。
>3. 建立候选列表：系统会在内存中初始化一个候选列表 $L$ 和一个已访问集合 $V$ 。起点 $s$ 会被首先放入 $L$ 中 。
>
>第二阶段：内存粗计算
>1. 生成查询查找表 (LUT)（具体是什么可以学习PQ量化的原理）：查询向量 $x_q$ 被切分，系统计算其与各个子空间聚类中心的距离，生成一张针对当前查询的距离查找表。
>2. 极速近似距离计算：对于当前候选列表 $L$ 中尚未被访问的节点（即属于 $L \setminus V$ 的节点），系统利用预先加载在内存中的乘积量化（PQ）压缩向量 $\tilde{x_p}$ 来计算近似距离 。
>3. 锁定目标 (Beam 选择)：算法不会像传统的贪婪搜索那样只挑出“最近的 1 个”节点去访问，而是选出距离 $x_q$ 最近的 $W$ 个未访问节点 。这里的 $W$ 就是 Beam 宽度，通常设置为 4 或 8 。细节考量：如果 $W=1$，算法就退化成了普通的贪婪搜索 。但如果 $W$ 设置得太大（例如 16 或更多），则会浪费计算资源和 SSD 带宽 。使用较小的 Beam 宽度（如 2、4、8）可以将 SSD 的负载率控制在 30-40%，从而保持极低的延迟 。
>
>第三阶段：访问SSD
>1. 并发 I/O 请求：系统同时向 SSD 发起获取这 $W$ 个节点数据的请求。读取少量（如 4 或 8 个）随机扇区的时间，与读取单个扇区的时间几乎是一样的，这就有效减少了往返磁盘（round-trips）的次数 。
>2. O(1) 地址计算：系统不需要在内存中查表找地址，而是直接通过节点 ID 乘以定长大小，计算出数据在 SSD 上的物理偏移量。
>3. 4KB 扇区对齐读取 (关键细节)：在 SSD 上，每个节点的数据布局是：全精度向量 $x_i$ 紧接着其最多 $R$ 个邻居的 ID 。在硬件层面，将一个 4KB 对齐的磁盘地址读入内存的开销，与读取 512 字节的开销并没有什么区别 。因此，当系统为了获取下一步的搜索方向去读取邻居 ID 列表时，它不产生任何额外磁盘读取开销地顺便把这 $W$ 个节点的全精度坐标也一起加载到了内存中 。
>
>第四阶段：全精度动态重排
>1. 替换近似值：PQ 压缩是损耗性的，会导致距离计算出现微小偏差 。算法此时会用这 $W$ 个节点的全精度向量，重新计算它们与 $x_q$ 之间真实、精确的欧氏距离 。
>2. 更新候选列表 $L$：将这 $W$ 个节点标记为已访问（加入集合 $V$）。把从磁盘读回来的这些节点的所有新邻居 ID 提取出来。用内存中的 PQ 查表法快速估算这些新邻居与 $x_q$ 的距离，并将它们插入候选列表 $L$ 中 。根据全精度距离（针对已访问节点）和 PQ 近似距离（针对新发现节点），动态截断并保留距离 $x_q$ 最近的 $L$ 个点，更新候选列表 。
>每次更新后的L都保证了：用全精度距离计算过的节点放入了V中，而L中既有全精度维护的最近点又有用压缩距离维护的最近点，因此当$L \setminus V = \emptyset$ 时，保证了L中所有的距离都是用全精度筛选过的。
>
>第五阶段：迭代收敛
>系统回到第二阶段，继续从更新后的候选列表 $L \setminus V$ 中挑出最新的 $W$ 个未访问节点，再次发起 SSD 请求。终止条件：当候选列表 $L$ 中最靠前的那些节点都已经全部被访问过（即 $L \setminus V = \emptyset$，无法再找到更近的节点）时，搜索循环终止 。返回 Top-K：最终，系统根据全精度坐标的真实距离，从列表 $L$ 中提取出最接近查询点 $x_q$ 的前 $k$ 个候选节点，并作为最终结果返回
# 评估
